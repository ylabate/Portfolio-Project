...
def populate(api_key=None):
    ...
    with app.app_context():
        ...
        if api_key:
            ...
            print(
                "No SteamGridDB API key provided. Using public Steam Store static asset fallbacks..."
            )
        ...
        for game_data in GAMES_TO_ADD:
            ...
            if existing:
                print(
                    f"Game '{name}' already exists in database. Updating images and metadata..."
                )
            ...
            if api_key:
                thumbnail, secondary_images = fetch_images_from_steamgriddb(
                    api_key, name
                )
            ...
            print(
                f"Added game: '{name}' with {len(secondary_images)} screenshots and stock keys."
            )
            db.session.commit()
